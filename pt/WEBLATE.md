...
* **Translated as files (pull requests):** the per-language prose pages
  (`about.<lang>.md`, `community.<lang>.md`, `no-permission.<lang>.md`,
  `non-software.<lang>.md`). Whole-page prose reads better when edited as one file, so
...
* **A new prose page translation:** add `about.<lang>.md` (etc.) in a pull request; see
...
2. In Weblate, add the language to each component. Weblate creates the
   ...
   them in.
3. Prose pages for the new language are added as files (`about.<code>.md`, …) when
